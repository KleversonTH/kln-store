...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    `SELECT c.id, c.produto_id, p.nome, p.preco, p.emoji, c.quantidade,
...
router.patch('/:produto_id', authMiddleware, async (req, res) => {
  ...
  try {
    if (quantidade <= 0) {
      ...
      await pool.query(
        'UPDATE carrinho SET quantidade = $1 WHERE cliente_id = $2 AND produto_id = $3',
        [quantidade, req.usuario.id, req.params.produto_id]
      );
    }
